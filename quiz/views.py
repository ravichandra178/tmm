...
class QuizTake(LoginRequiredMixin,FormView):
    ...
    def get_form(self, form_class=QuestionForm):
        ...
        if self.question.__class__ is DDQuestion:
            form_class = DDQForm
    ...
    def form_valid_user(self, form):
        ...
        num = self.question.id
        
        if self.question.qtype == 'ddq':
            y = DDAnswer.objects.filter(question_id = self.question.id)
            list1 = []
            for i in y:
                list1.append(i.content)
            if guess not in list1:
                r = DDAnswer(content = guess,question_id = self.question.id)
                r.save()
            is_correct = self.question.check_if_correct(guess,num)
            #x = DDAnswer.objects.filter(question_id = self.question.id,correct = True)
            #for j in x:
        ...
        elif self.question.qtype == 'fitb':
            y = FITBAnswer.objects.filter(question_id = self.question.id)
            list1 = []
            for i in y:
                list1.append(i.content)
            if guess not in list1:
                r = FITBAnswer(content = guess,question_id = self.question.id)
                r.save()
            is_correct = self.question.check_if_correct(guess,num)
            #x = FITBAnswer.objects.filter(question_id = self.question.id,correct = True)
            #for j in x:
                #correct_ans = j.content
            z = FITBAnswer.objects.filter(correct = False)
            z.delete()
            
        elif self.question.qtype == 'essay':
            y = EssayAnswer.objects.filter(question_id = self.question.id)
            list1 = []
            for i in y:
                list1.append(i.content)
            if guess not in list1:
                r = EssayAnswer(content = guess,question_id = self.question.id)
                r.save()
            is_correct = self.question.check_if_correct(guess,num)
            #x = EssayAnswer.objects.filter(question_id = self.question.id,correct = True)
            #for j in x:
                #correct_ans = j.content
            ...
            #x = Answer.objects.filter(question_id = self.question.id,correct = True)
            #for j in x:
                #correct_ans = j.content
        ...
        #self.correctans = {'correct_ans' : correct_ans}
